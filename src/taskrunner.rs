...
impl TaskRunner {
    ...
    pub fn state(&self) -> TaskState {
        todo!("report the task's current state")
    }

    /// How many times the task has been restarted.
    #[must_use]
    pub fn restart_count(&self) -> u32 {
        todo!("return the restart counter")
    }
    ...
    pub fn start(&mut self) -> Result<()> {
        todo!("resolve the driver and start the task")
    }
    ...
    pub fn handle_exit(&mut self, success: bool) -> bool {
        todo!("decide restart vs terminal for success={success} against the policy")
    }
}
